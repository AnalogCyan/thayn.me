// Sends Bridgy publish webmentions for Mastodon and Bluesky syndication

import { sanitizeExternalUrl } from "./sanitize-url.js";
...
export function normalizeSyndicateTargets(raw, additionalTargets = {}) {
  const targets = new Set();
  const addTarget = (value) => {
    const key = normalizeTarget(value);
    if (!key) return;
    if (!BRIDGY_PUBLISH_TARGETS[key] && !additionalTargets[key]) return;
    targets.add(key);
  };
...
export function normalizeSyndicationMap(raw) {
  ...
  if (Array.isArray(raw)) {
    raw.forEach((entry) => {
      ...
      if (key && safeUrl) map[key] = safeUrl;
    });
    return map;
  }
  ...
  if (typeof raw === "object") {
    Object.entries(raw).forEach(([key, value]) => {
      ...
      if (typeof value === "string") {
        const safeValue = sanitizeExternalUrl(value);
        if (safeValue) map[normalizedKey] = safeValue;
        return;
      }
      if (Array.isArray(value) && value[0]) {
        const safeValue = sanitizeExternalUrl(value[0]);
        if (safeValue) map[normalizedKey] = safeValue;
        return;
      }
      if (typeof value === "object") {
        const url = value.url || value.href;
        const safeValue = sanitizeExternalUrl(url);
        if (safeValue) map[normalizedKey] = safeValue;
      }
    });
  }
...
export async function sendBridgyWebmention({
  ...
  try {
    ...
    // Bridgy refuses a page it already published with a 400 that names the
    // earlier post, which is the answer a confirm is looking for
    const alreadyPublished = sanitizeExternalUrl(payload?.original?.url);
    const syndicatedUrl =
      payload?.url ||
      payload?.location ||
    ...
    return {
      ok: res.ok || Boolean(alreadyPublished),
      status: res.status,
      syndicatedUrl,
    };
  } catch (err) {
...
function yamlScalar(value) {
  if (value instanceof Date) {
    ...
    if (iso.endsWith("T00:00:00.000Z")) {
      return JSON.stringify(iso.slice(0, 10));
    }
